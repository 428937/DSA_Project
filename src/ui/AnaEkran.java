...
public class AnaEkran extends JFrame {
    ...
    private void arayuzuOlustur() {
        ...
        ekleButonu = yeniButonOlustur("Yeni İlan Ekle", new Color(76, 175, 80), e -> ilanEkleIslem());
        guncelleButonu = yeniButonOlustur("Güncelle", new Color(33, 150, 243), e -> ilanGuncelleIslem());
        silButonu = yeniButonOlustur("Sil", new Color(244, 67, 54), e -> ilanSilIslem());
        favoriButonu = yeniButonOlustur("Favori Ekle/Çıkar", new Color(255, 152, 0), e -> favoriToggle());
        gerialButonu = yeniButonOlustur("Geri Al (Ctrl+Z)", new Color(158, 158, 158), e -> gerialYap());
        yineleButonu = yeniButonOlustur("Yinele (Ctrl+Y)", new Color(158, 158, 158), e -> yineleYap());
        ...
        tabloModel = new DefaultTableModel(new String[]{"ID", "Ad", "Konum", "Fiyat (₺)", "Alan (m²)", "Tip", "Oran (₺/m²)"}, 0) {
    ...
    private void tuşKısayollariniYükle() {
        KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(e -> {
            if (e.getID() == KeyEvent.KEY_PRESSED) {
                if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_Z) { gerialYap(); return true; }
                if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_Y) { yineleYap(); return true; }
                if (e.getKeyCode() == KeyEvent.VK_DELETE && secilenEmlak != null) { ilanSilIslem(); return true; }
            }
            return false;
        });
    }
    ...
    private void ilanEkleIslem() {
        ...
        if (dialog.isKaydedildi()) {
            ...
            if (yonetici.emlakEkle(yeniEmlak.getAd(), yeniEmlak.getKonum(), yeniEmlak.getFiyat(), yeniEmlak.getAlan(), yeniEmlak.getTip(), yeniEmlak.getAciklama())) {
                listeyiYenile();
                verileriBilgisiniGuncelle();
                JOptionPane.showMessageDialog(this, "İlan başarıyla eklendi.", "Başarılı", JOptionPane.INFORMATION_MESSAGE);
            }
        }
    }
    
    private void ilanGuncelleIslem() {
        if (secilenEmlak == null) { JOptionPane.showMessageDialog(this, "Lütfen güncellenecek bir ilan seçiniz.", "Uyarı", JOptionPane.WARNING_MESSAGE); return; }
        IlanGuncelleDialog dialog = new IlanGuncelleDialog(this, secilenEmlak);
        dialog.setVisible(true);
        if (dialog.isGuncellendi()) {
            yonetici.emlakGuncelle(secilenEmlak.getId(), secilenEmlak.getAd(), secilenEmlak.getKonum(), secilenEmlak.getFiyat(), secilenEmlak.getAlan(), secilenEmlak.getTip(), secilenEmlak.getAciklama());
            listeyiYenile();
            verileriBilgisiniGuncelle();
            JOptionPane.showMessageDialog(this, "İlan başarıyla güncellendi.", "Başarılı", JOptionPane.INFORMATION_MESSAGE);
        }
    }
    
    private void ilanSilIslem() {
        if (secilenEmlak == null) { JOptionPane.showMessageDialog(this, "Lütfen silinecek bir ilan seçiniz.", "Uyarı", JOptionPane.WARNING_MESSAGE); return; }
        int sonuc = JOptionPane.showConfirmDialog(this, "İlanı silmek istediğinizden emin misiniz?", "Onay", JOptionPane.YES_NO_OPTION);
        if (sonuc == JOptionPane.YES_OPTION) {
            yonetici.emlakSil(secilenEmlak.getId());
            secilenEmlak = null;
            listeyiYenile();
            verileriBilgisiniGuncelle();
            JOptionPane.showMessageDialog(this, "İlan başarıyla silindi.", "Başarılı", JOptionPane.INFORMATION_MESSAGE);
        }
    }
    
    private void favoriToggle() {
        if (secilenEmlak == null) { JOptionPane.showMessageDialog(this, "Lütfen bir ilan seçiniz.", "Uyarı", JOptionPane.WARNING_MESSAGE); return; }
        if (secilenEmlak.isFavosilerde()) { yonetici.favoriKaldir(secilenEmlak.getId()); } else { yonetici.favoriEkle(secilenEmlak.getId()); }
        listeyiYenile();
    ...
    private void gerialYap() {
        if (!yonetici.gerialYapilabilir()) { JOptionPane.showMessageDialog(this, "Geri alınacak işlem yok.", "Bilgi", JOptionPane.INFORMATION_MESSAGE); return; }
        yonetici.gerialYap();
        listeyiYenile();
        favoriPanel.listeyiYenile();
    ...
    private void yineleYap() {
        if (!yonetici.yineleYapilabilir()) { JOptionPane.showMessageDialog(this, "Yinelenecek işlem yok.", "Bilgi", JOptionPane.INFORMATION_MESSAGE); return; }
        yonetici.yineleYap();
        listeyiYenile();
        favoriPanel.listeyiYenile();
        aramaPanel.listeyiYenile();
    ...
    public void listeyiYenile() {
        ...
        for (Emlak emlak : yonetici.tumEmlaklar()) {
            tabloModel.addRow(new Object[]{ emlak.getId(), emlak.getAd(), emlak.getKonum(), String.format("%.2f", emlak.getFiyat()), String.format("%.2f", emlak.getAlan()), emlak.getTip(), String.format("%.2f", emlak.getFiyatAlanOrani()) });
        }
    }
    ...
    private void guncelleSeciliLabel() {
        if (secilenEmlak != null) { secilenLabel.setText(String.format("Seçili: %s (%s) - Fiyat: %.0f₺", secilenEmlak.getAd(), secilenEmlak.getKonum(), secilenEmlak.getFiyat())); } else { secilenLabel.setText("Seçili İlan: Yok"); }
    }
